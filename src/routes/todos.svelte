<script context="module">
  export function preload({ params, query }) {
    return this.fetch('todo').then(r => r.json())
  }
</script>
...
<script>
  ...
  async function createNew(event) {
    if (event.which === ENTER_KEY) {
      ...
      const response = await this.fetch('todo', { method: 'POST', body: JSON.stringify({ todoText }) })
      const { id, date, success } = await response.json()
      if (success) {
        items = items.concat({ id, content: todoText, created: date, completed: false })
        event.target.value = ''
      }
    }
  }
  ...
  async function handleEdit(event) {
    if (event.which === ENTER_KEY) {
      ...
      const toEdit = items.find(({ id }) => id === todoID)
      if (toEdit) {
        const todoText = event.target.value
        const response = await this.fetch('todo', { method: 'PATCH', body: JSON.stringify({ todoText, todoID }) })
        const { success } = await response.json()
      ...
      {#each filtered as item (item.id)}
        <li class="{item.completed ? 'completed' : ''} {editing === item.id ? 'editing' : ''}">
          <div class="view">
            ...
            <label on:dblclick="{() => editing = item.id}">{item.content}</label>
            <button on:click="{() => remove(item.id)}" class="destroy">Delete</button>
          </div>
